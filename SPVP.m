...
% User-defined knowns
Vinf = 100;                                                                   % Freestream velocity [] (just leave this at 1)
AoA  = 0;                                                                   % Angle of attack [deg]
NACA = '0010';                                                              % NACA airfoil to load [####(#)]
rho = 1.225;
miu = 0.0000181206;
nu = miu/rho;
Re = Vinf/nu
...
while any(error > 0.000001)
    ...
    %% Thwaites
    [sep_u ,sep_l,theta_thwaites,disp_t,H_thwaites,Cf_thwaites,vtran] = THWAITES(Vt,XC,YC,i_s,nu);

    %% Calculate Error    
    if iter == 1
        disp_t_old = Cf_thwaites;
        iter = iter + 1;
    else
        error = abs(Cf_thwaites-disp_t_old)/Cf_thwaites;
        disp_t_old = Cf_thwaites;
        iter = iter + 1;
    end
end
...
CN = -Cp.*S.*sin(beta);
CA = -Cp.*S.*cos(beta)+Cf_thwaites.*S;
...
if (flagPlot(4) == 1)
    ...
    pXl = plot(xFoilX(midIndX+1:end),xFoilCP(midIndX+1:end),'r-',...        % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(1:midIndS),Cp(1:midIndS),'s','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(midIndS+1:end),Cp(midIndS+1:end),'s',...                 % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    psep = plot([XC(sep_u) XC(sep_l)],[Cp(sep_u) Cp(sep_l)],'m+','LineWidth',2);
    legend([pXu,pXl,pVu,pVl,psep],...                                            % Show legend
           {'XFOIL Upper','XFOIL Lower','Thwaites Upper','Thwaites Lower','Separation Point'});
...
if (flagPlot(7) == 1)
    ...
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    pdisp_u=plot(XC(i_s:numPan),YC(i_s:numPan)+disp_t(i_s:numPan));   % Plot Displacement Thickness
    pdisp_l=plot(XC(1:i_s-1),YC(1:i_s-1)-disp_t(1:i_s-1));            % Plot Displacement Thickness
    psep = plot([XC(sep_u) XC(sep_l)],[YC(sep_u) YC(sep_l)],'r+');
    pfoil = plot(XB,YB,'k--');                                                        % Plot airfoil as black polygon
    legend([pfoil pdisp_u pdisp_l psep],{'Airfoil','Upper Boundary Layer','Lower Boundary Layer','Separation Point'})
    xlabel('X Units');                                                      % Set X-label
    ylabel('Y Units');                                                      % Set Y-label
    ...
    pXl = plot(BLx(midIndX+1:end),Dstar(midIndX+1:end),'r-',...             % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(sep_l:i_s-1),disp_t(sep_l:i_s-1),'s','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(i_s:sep_u),disp_t(i_s:sep_u),'s',...                         % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    psep = plot([XC(sep_u) XC(sep_l)],[disp_t(sep_u) disp_t(sep_l)],'m+','LineWidth',2);
    legend([pXu,pXl,pVu,pVl,psep],...                                            % Show legend
           {'XFOIL Upper','XFOIL Lower','Thwaites Upper','Thwaites Lower','Separation Point'});
    xlabel('X Coordinate');                                                 % Set X-label
    ...
    pXl = plot(BLx(midIndX+1:end),theta(midIndX+1:end),'r-',...             % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(sep_l:i_s-1),theta_thwaites(sep_l:i_s-1),'s','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(i_s:sep_u),theta_thwaites(i_s:sep_u),'s',...                         % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    psep = plot([XC(sep_u) XC(sep_l)],[theta_thwaites(sep_u) theta_thwaites(sep_l)],'m+','LineWidth',2);
    legend([pXu,pXl,pVu,pVl,psep],...                                            % Show legend
           {'XFOIL Upper','XFOIL Lower','Thwaites Upper','Thwaites Lower','Separation Point'});
    xlabel('X Coordinate');                                                 % Set X-label
    ...
    pXl = plot(BLx(midIndX+1:end),H(midIndX+1:end),'r-',...             % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(1:i_s-1),H_thwaites(1:i_s-1),'ks-','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(i_s:end),H_thwaites(i_s:end),'ks-',...                         % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    ...
    pXl = plot(BLx(midIndX+1:end),Cf(midIndX+1:end),'r-',...             % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
    pVl = plot(XC(sep_l:i_s-1),Cf_thwaites(sep_l:i_s-1),'s','MarkerFaceColor','r');     % Plot Cp for upper surface of airfoil from SPM
    pVu = plot(XC(i_s:sep_u),Cf_thwaites(i_s:sep_u),'s',...                         % Plot Cp for lower surface of airfoil from SPM
                    'MarkerFaceColor','b');
    psep = plot([XC(sep_u) XC(sep_l)],[Cf_thwaites(sep_u) Cf_thwaites(sep_l)],'m+','LineWidth',2);
    legend([pXu,pXl,pVu,pVl,psep],...                                           % Show legend
           {'XFOIL Upper','XFOIL Lower','Thwaites Upper','Thwaites Lower','Separation Point'});
    xlabel('X Coordinate');                                                 % Set X-label
